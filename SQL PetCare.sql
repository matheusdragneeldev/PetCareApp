...
DROP VIEW IF EXISTS perfil_info_clinica;
CREATE VIEW perfil_info_clinica AS
SELECT cl.id_clinica, cl.foto ,cl.nome, cl.descricao, cl.telefone, cl.funcionamento, e.logradouro, e.numero, e.complemento ,e.bairro, e.cidade, e.estado
FROM clinica cl
INNER JOIN endereco e ON cl.id_clinica = e.fk_id_login;
...
SELECT 
    ...
    c.foto AS foto_clinica, 
    f.feedback_mensagem, 
    f.dt_hr_feedback, 
    f.id_feedback
FROM login AS l
LEFT JOIN tutor AS t ON l.id_login = t.id_tutor
LEFT JOIN clinica AS c ON l.id_login = c.id_clinica
...
DROP VIEW IF EXISTS vw_lembretes_vacinas;
CREATE VIEW vw_lembretes_vacinas AS
SELECT
    p.nome AS nome_do_pet,
    cv.nome_vacina AS nome_da_vacina,
    cv.dt_vacinacao AS data_da_vacina,
    l.email AS email_do_tutor
FROM
    cartao_vacinacao cv
JOIN
    pet p ON cv.fk_id_pet = p.id_pet
JOIN
    tutor t ON p.fk_id_tutor = t.id_tutor
JOIN
    login l ON t.fk_id_login = l.id_login
WHERE
    l.tipo_user = 'Tutor';
